...
mean_by_rep_long_near_far_combined <- long_data %>%
        filter(hidden_pa_img_type %in% c('far','near')) %>%
        droplevels() %>%
        group_by(ptp,
                 counterbalancing,
...
mean_by_rep_long_all_types <- bind_rows(mean_by_rep_long_all_pa,
                                        ...
                                        mean_by_rep_long_near_far_combined,
...
data_summary_all_pas <- long_data %>%
        group_by(ptp,
                 counterbalancing,
                 condition) %>%
        summarise(block_1_mouse_error_mean   = mean(mouse_error[cur_data()$block==1],na.rm=T),
                  block_1_mouse_error_sd     = sd(mouse_error[cur_data()$block==1],na.rm=T),
                  block_1_rt_mean            = mean(rt[cur_data()$block==1],na.rm=T),
                  block_1_rt_sd              = sd(rt[cur_data()$block==1],na.rm=T),
                  block_2_mouse_error_mean   = mean(mouse_error[cur_data()$block==2],na.rm=T),
...
### Near and Far PAs Combined ===============================
data_summary_near_far_pas_combined <- long_data %>%
        filter(hidden_pa_img_type %in% c('near','far')) %>%
        droplevels() %>%
        group_by(ptp,
                 counterbalancing,
...
data_summary <- bind_rows(data_summary_all_pas,
                          ...
                          data_summary_near_far_pas_combined)
