...
data_for_bf_schema_6_0_far_vs_near_lr <- data_summary %>%
        ...
        pivot_wider(id_cols = c(ptp,
                    ...
                    names_from = hidden_pa_img_type,
                    values_from = learning_rate_two_param_no_outlier) %>% 
        mutate(far_minus_near = far - near) %>% View()
...
data_for_bf_near_l_vs_c_lr <- data_summary %>%
        ...
        pivot_wider(id_cols = c(ptp,
                    ...
                    values_from = learning_rate_two_param_no_outlier) %>% 
        mutate(schema_l_minus_schema_6_0 = schema_l - schema_6_0) %>% 
        select(schema_l_minus_schema_6_0) %>% .[[1]]
...
data_for_bf_near_schema_6_0_vs_ic_lr <- data_summary %>%
        filter(hidden_pa_img_type == 'near') %>%
        droplevels() %>%
        ...
        pivot_wider(id_cols = c(ptp,
                    ...
                    names_from = condition,
                    values_from = learning_rate_two_param_no_outlier) %>% 
        mutate(schema_6_0_minus_schema_ic = schema_6_0 - schema_ic) %>% 
        select(schema_6_0_minus_schema_ic) %>% .[[1]]


null_interval <- c(0,Inf)

bf_near_schema_6_0_vs_ic_lr <- reportBF(ttestBF(
        data_for_bf_near_schema_6_0_vs_ic_lr,
        nullInterval = null_interval
)[1],4)
